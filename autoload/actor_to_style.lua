...
script_description = tr("使用txt导入多人轴，将说话人从说话人栏转移到样式栏（前提是说话人要与样式要一致）")
script_author = "Yiero"
...
re = function(subs, selected_lines)
    ...
    fg_GUI = {{
        ...
        width = 8,
        class = "label",
        label = "用于快速将说话人匹配至样式栏，并删除说话人；也可用于文本的导入(兼容全角冒号)"
    }, {
        x = 1,
        y = 1,
        class = "label",
    ...
    fg, fg_res = aegisub.dialog.display(fg_GUI, {"OK", "Add", "Cancel"}, {
        save = "OK",
        close = "Cancel"
    })
    ...
    if fg == "Add" then
        ...
        txt, txt_res = aegisub.dialog.display(txt_GUI, {"OK", "Cancel"}, {
            save = "OK",
            close = "Cancel"
        })
        ...
        for v in io.lines(f) do
            ...
            for char in _G.unicode.chars(v) do
                ...
                if unicode.codepoint(char) < 0 then
                    table.insert(_v,
                        string.format(
                            "Runtime error in \"%s\" (line %s, character %s) \nUTF-8(Encoding) expect, got illegal character (%s)",
                            f, #lines, c, unicode.codepoint(char)))
                end
            end
        end
